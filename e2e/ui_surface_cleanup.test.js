...
test.describe('UI Surface Cleanup', () => {
  ...
  test('rest and level-up modals are explicit about risk and partial implementation', async ({ page }) => {
    ...
    await expect(page.locator('#lu-feat-note')).toContainText('Feat choice is not implemented in this build');
    await expect(page.locator('#btn-confirm-level-up')).toHaveText('Confirm Ability Score Increase');
  });
});
